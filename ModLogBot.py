...
configFile = f"{config_folder_path}config.yml"
...
async def check_db_size():
    ...
    if db_size > config.db_size_warning_threshold:
        ...
        for owner_id in bot_owners:
            ...
            await owner_user.send(
                f"Database size of {db_size:.2f}MB exceeds warning threshold of {config.db_size_warning_threshold}MB"
            )
...
async def on_audit_log_entry_create(entry):
    guild = entry.guild
    server = config.servers.get(guild.id) # or None
    if not server:
        print(f"Guild '{guild.name}' ({guild.id}) is not configured. Ignoring audit log event.")
        return # Server not found
    
    if server.log_channel_id:
        log_channel = guild.get_channel(server.log_channel_id)
    ...
    if entry.action == discord.AuditLogAction.ban:
        ...
        if entry.extra.channel.id in server.ignored_channels:
...
async def handle_guild_message(message: discord.Message):
    ...
    server = config.servers.get(message.guild.id) # or None
    if not server:
        #print(f"Guild '{message.guild.name}' ({message.guild.id}) is not configured. Ignoring channel message.") # Disabled to not spam logs on misconfiguration
        return # Server not found

    # Process auto message removal channels
    await handle_auto_message_removal(server, message)

# Function for handling the "auto message removal" feature of the bot
# Called when any chat message is sent on a server
async def handle_auto_message_removal(server: Server, message: discord.Message) -> None:
    for auto_message_removal in server.auto_message_removals:
...
async def warn(interaction: discord.Interaction, user: discord.Member, reason: str, attachment: discord.Attachment=None) -> None:
    ...
    server = config.servers.get(guild.id) # or None
    if not server:
        print(f"Guild '{guild.name}' ({guild.id}) is not configured. Ignoring warn command.")
        return # Server not found
...
async def history(interaction: discord.Interaction, user: discord.Member | discord.User) -> None:
    ...
    server = config.servers.get(guild.id) # or None
    if not server:
        print(f"Guild '{guild.name}' ({guild.id}) is not configured. Ignoring history command.")
        return # Server not found
    ...
    for item in user_history:
        ...
        if action_text:
            if guild.id and server.log_channel_id and item.log_message_id:
                embed.description += f"\n**{action_text}:**  https://discord.com/channels/{guild.id}/{server.log_channel_id}/{item.log_message_id}"
            else:
                embed.description += f"\n**{action_text}**"
...
async def report(
    ...
    if not server.isdigit() or int(server) not in config.servers:
    ...
    serverConfig = config.servers.get(server) # or None
    if not serverConfig:
        print(f"Guild ID ({server}) is not configured. Ignoring report command.")
        return # Server not found

    if not serverConfig.report_channel_id:
        print(f"Report channel not set for server '{serverConfig.name}' ({server}).")
        await interaction.response.send_message("Report channel not set for this server.")
        return

    report_channel = bot.get_channel(serverConfig.report_channel_id)
    if not report_channel:
        print(f"Report channel with ID {serverConfig.report_channel_id} not found in server '{serverConfig.name}' ({server}).")
    ...
    if serverConfig.report_role_ping_id:
        await report_channel.send(f"<@&{serverConfig.report_role_ping_id}> Member Report", embed=embed)
    else:
...
async def server_autocomplete(interaction: discord.Interaction, current: str) -> List[Choice[str]]:
    report_servers = [id for id, server in config.servers.items() if server.report_channel_id is not None]
